...
stmt_t *new_stmt(expr_t *expr) {
    ...
    ret->type = EXPR;
    ret->data = (void*)expr;
    parselog("Expression statement created");
    return ret;
}
...
stmt_t *new_asignment_stmt(asign_t *asign) {
    ...
    ret->type = ASIGN;
    ret->data = (void*)asign;
    parselog("Assignment statement created");
    return ret;
}
...
stmt_list_t *empty_stmt_list() {
    parselog("Creating statement list");
...
stmt_list_t *append_stmt_list(stmt_list_t *list, stmt_t *stmt) {
    ...
    ++(list->size);
    parselog("Adding statement to list of size %d", list->size);
    return list;
}
